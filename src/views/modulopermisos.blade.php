...
@section('content')
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    @foreach ($permisos as $permiso)
                        <div>
                            <input type="checkbox" value="{{ $modulo->moduloid . '-' . $permiso->permisoid }}"
                                id="mp{{ $modulo->moduloid . '-' . $permiso->permisoid }}" name="permiso[]"
                                class="chk{{ $modulo->moduloid }}"
                                {{ isset($modulopermisos[$modulo->moduloid][$permiso->permisoid]) ? 'checked="true"' : '' }}>
    ...
    @endforeach
    <div class="col-sm-12">{{ Form::submit('Guardar', ['class' => 'btn btn-primary']) }}</div>
    {{ Form::close() }}
    <script>
