...
@section('content')
    ...
    @endif
    {{ Form::open(['url' => 'cancerbero/asignar', 'class' => 'form-inline']) }}
    <div class="col-sm-12">
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    @foreach ($val['permisos'] as $permiso)
                        <div class="col-sm-2">
                            <input type="checkbox" value="{{ $permiso['id'] }}" id="mp{{ $permiso['id'] }}"
                                name="modulopermisos[]" class="chks chk{{ $val['moduloid'] }}"
                                {{ in_array($permiso['id'], $rolmodulopermisos) ? 'checked="true"' : '' }}>
                            {{ $permiso['nombre'] }}
                        </div>
                    @endforeach
                </div>
    ...
    @endforeach
    <div class="col-md-12">{{ Form::submit('Guardar', ['class' => 'btn btn-primary']) }}</div>
